function obj = GridAvInterp(fname,obj,varargin)
% obj = GridAvInterp(fname,obj,'K',K,'type',type);
% GridData: Uses the cell-averaged approach to interpolate the nodes
%           on the unstructured grid to the DEM.          
%       Input :   fname - A filename of the netcdf DEM
%                   obj - A msh structure/object containing:
...
%
%      Output : obj     - A mesh structure/object with the following updated:
...
%% Let's read the LON LAT of DEM
DEM_XA = double(ncread(fname,'lon'));
DEM_YA = double(ncread(fname,'lat'));
...
DEM_Z = single(ncread(fname,zvarname,...
...
if strcmp(interp,'CA')
    ...
    %% Get griddedInterpolant for non-CA interp
...
% New method of averaging absolute values of slope before multiplying
