...
%
% msh class notes : a msh class obj contains parameters like the vertices
% (p), element connectivity table (t), depths (b), and a fort.13 (f13)
% ADCIRC style attribute file struct that are required for this routine. 
% For non-ADCIRC applications one can change the output from f13 to their own.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if strcmp(type,'tensor')
   % Compute gradients of J from Nm and bathymetry B, and grid  
...
if isempty(N_filename)
    ...
    Nb_t = Nb;
    if strcmp(crit,'Nmw')
        % depth-averaged with weights linearly decreasing from seabed
        Nb_t = Nmw;
    end
...
% Make coefficient zero at zero slope locations
...
%
%% Make into f13 struct for the output
if isempty(obj.f13)
