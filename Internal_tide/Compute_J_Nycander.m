...
%     bathyfile - filename of gridded bathymetry data (not we would want
...
if parallel > 0
    ...
    minl = min(XX(idv,2)); maxl = max(XX(idv,2)); dl = maxl - minl; njj =0;
    for idx = 1:num_p
        if idx < num_p
            maxp = minl + dl/num_p/4;
        else
            maxp = maxl;
        end
    ...
    for idx = 1:num_p
        I = I_c{idx};
        iel = ielm{idx};
        fJ(idx) = parfeval(Pool,@get_J,2,XX(idv(I),:),-B(idv(I)),a(idv(I)),...
...
function [J,dJ] = get_J(XX,h,a,xc,xcub,ycub,hcub,jc,rcutfac,RefObj2dCub)
    ...
    for i = 1: nj
        ...
        if mod(i,ceil(nj/25)) == 0
            fprintf('node = %d ; nsum = %d ; amv = %f (km) \n', ...
                    i, length(ielm), amv/1000 ) ;
        end
        % Bottom topography (increasing upward)
        bs = h(i) ;
        hs = hcub(:,ielm) - bs ; % I would use: hs = hcub(:,ielm)
