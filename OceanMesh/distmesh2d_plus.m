function [p,t] = distmesh2d_plus(fd,fh,h0,bbox,p,pfix,itmax,plot_on,nscreen)
...
%   topological clean-up, and handling of fixp.
...
if isempty(p)
    ...
    % Redistribute the vertices of the contours  
    if ~isempty(pfix)
        pfix=pfix(feval(fd,pfix,0)<geps,:);              % Keep only d<0 points
        r0=1./feval(fh,pfix).^+2;                        % Probability to keep point
        max_r0 = 1/h0^2;
        pfix=pfix(rand(size(pfix,1),1) < r0/max_r0,:);   % Rejection method
    end
...
while 1
    ...
    if max(sqrt(sum((p-pold).^2,2))/h0)> ttol            % Any large movement?
        p=[pfix;p];                                      % Append fixed points
        p=unique(p,'rows','stable');                     % Ensure points are unique.
        ...
        if plot_on >= +1 && (mod(it,nscreen)==0 || it == +1)
            ...
            if ~isempty(pfix),hold on; plot(pfix(:,1),pfix(:,2),'r.');end;
            drawnow
        end
    end
    ...
    if mod(it,imp)==0
        if save_qual(it-1,1) - save_qual(it-2,1)/deltat < +1e-2
            ...
            p(nn,:)= []; p=[p; pst];                   %-->p is duplicated here but 'setdiff' above
            pold = inf; it = it+1;
            continue;
        end
    end
