...
PROGRAM FigureGen
        ...
        IF(MyRank.EQ.0)THEN
            ...
            WRITE(*,'(A)') " "
            WRITE(*,'(A)') "--------------------------------------"
            WRITE(*,'(A)') "FigureGen 49                2012/06/06"
            WRITE(*,'(A)') " "
...
!        ENDIF
        ...
        IF(MyRank.EQ.0)THEN
            ...
            IF((IfPlotFilledContours.GT.0).OR.(IfPlotContourLines.GT.0))THEN
                IF(ContourFileType(1:13).EQ."ADCIRC-OUTPUT")THEN
                    ...
                    ELSEIF(TRIM(ContourFileFormat1).EQ."NETCDF")THEN
                        ...
                        ALLOCATE(Time(NumRecs))
                        CALL Check(NF90_GET_VAR(NC_ID,NC_Var,Time))
...
#ifdef CMPI
            DO I=1,NumProcs-1         
                CALL MPI_SEND(timeL,1,MPI_DOUBLE,I,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE,IERR)
            ENDDO
#endif
        ...
        IF(MyRank.NE.0)THEN
            ...
            IF(JunkI.NE.0)THEN
               STOP
            ENDIF
            CALL MPI_RECV(timeL,1,MPI_DOUBLE,0,0,MPI_COMM_WORLD,MPI_STATUS_IGNORE,IERR)
        ENDIF
        ...
        IF(NumRecords.EQ.0)THEN
            ...
            IF (ContourFileType(14:16).eq.'amp'.or.ContourFileType(14:16).eq.'phs') THEN
               ...
               ELSEIF(IfPlotVectors.GT.0)THEN
                   CALL Check(NF90_OPEN(TRIM(VectorFile),NF90_NOWRITE,NC_ID))
               ENDIF        
               CALL Check(NF90_INQ_DIMID(NC_ID,'num_const',NC_DimIDs(2)))
               CALL Check(NF90_INQUIRE_DIMENSION(NC_ID,NC_DimIDs(2),len=NumRecs))
               CALL Check(NF90_CLOSE(NC_ID))
            ENDIF
#endif
            NumRecords = max(1,NumRecs)
        ...
        ELSE
            ...
            outer2: DO I=1,NumRecords*NumSubRecords
                ...
                IF(WorkingRecord.GT.0)THEN
                    ...
                    IF((IfPlotFilledContours.GT.0).OR.(IfPlotContourLines.GT.0).OR.(IfPlotVectors.GT.0))THEN
        ...
        DO K=1,3
            ...
            ELSE
                IF(IfGoogle.EQ.0)THEN
                    ...
                    !WRITE(UNIT=15,FMT='(A,I3,A,I3,A,I3)') "B  ",        &
                    !                    NINT(GMTColors(1)%Red2),"  ",   &
                    !                    NINT(GMTColors(1)%Green2),"  ", &
                    !                    NINT(GMTColors(1)%Blue2)
                ELSE
                    WRITE(UNIT=15,FMT='(A)') "B  255  255  255"
                ENDIF
            ENDIF
            ...
            IF(ContourFileType(1:13).EQ."ADCIRC-OUTPUT")THEN
                ...
                ELSEIF(TRIM(ContourFileFormat1).EQ."NETCDF")THEN
                    ...
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ...
                        NC_File = 63
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID1,'zeta_max',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 633
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID1,'vel_max',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 634
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID1,'wind_max',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 635
                    ENDIF
                    ...
                    ELSE
                        ...
                        ELSEIF(TRIM(ContourFileFormat1).EQ."NETCDF")THEN
                            ...
                            IF(NC_File.EQ.63)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'zeta',NC_Var))                           
                            ELSEIF(NC_File.EQ.633)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'zeta_max',NC_Var))
                            ELSEIF(NC_File.EQ.634)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'vel_max',NC_Var))
                            ELSEIF(NC_File.EQ.635)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'wind_max',NC_Var))
                            ELSEIF(NC_File.EQ.64)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'u-vel',NC_Var))
                            ELSEIF(NC_File.EQ.73)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'pressure',NC_Var))
                            ELSEIF(NC_File.EQ.74)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'windx',NC_Var))
                            ELSEIF(NC_FILE.EQ.301)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'swan_HS',NC_Var))
                            ELSEIF(NC_FILE.EQ.302)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'swan_DIR',NC_Var))
                            ELSEIF(NC_FILE.EQ.303)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'swan_TPS',NC_Var))
                            ELSEIF(NC_FILE.EQ.307)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'swan_TMM10',NC_Var))
                        ...
                        IF(TRIM(ContourFileFormat1).EQ."NETCDF")THEN
                            ...
                            IF(NC_File.EQ.63)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'zeta',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID1,NC_Var,U1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.633)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'zeta_max',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID1,NC_Var,U1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.64)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'u-vel',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID1,NC_Var,U1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                                CALL Check(NF90_INQ_VARID(NC_ID1,'v-vel',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID1,NC_Var,V1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.73)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID1,'pressure',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID1,NC_Var,U1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.74)THEN
            ...
            IF(ContourFileType(1:13).EQ."ADCIRC-OUTPUT")THEN
                ...
                ELSEIF(TRIM(ContourFileFormat2).EQ."NETCDF")THEN
                    ...
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ...
                        NC_File = 63
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'zeta_max',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 633
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'vel_max',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 634
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'wind_max',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 635
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'u-vel',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 2
                        NC_File = 64
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'pressure',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 73
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'windx',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 2
                        NC_File = 74
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'swan_HS',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 301
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'swan_DIR',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 302
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'swan_TPS',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 303
                    ENDIF
                    NC_Status = NF90_INQ_VARID(NC_ID2,'swan_TMM10',NC_Var)
                    IF(NC_Status.EQ.NF90_NOERR)THEN
                        ContourFileNumCols = 1
                        NC_File = 307
                    ENDIF
                ...
                loopminmax2: DO J=1,NumRecsLocal 
                    ...
                    ELSE
                        ...
                        ELSEIF(TRIM(ContourFileFormat2).EQ."NETCDF")THEN
                            ...
                            IF(NC_File.EQ.63)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'zeta',NC_Var))
                            ELSEIF(NC_File.EQ.633)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'zeta_max',NC_Var))
                            ELSEIF(NC_File.EQ.634)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'vel_max',NC_Var))
                            ELSEIF(NC_File.EQ.635)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'wind_max',NC_Var))
                            ELSEIF(NC_File.EQ.64)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'u-vel',NC_Var))
                            ELSEIF(NC_File.EQ.73)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'pressure',NC_Var))
                            ELSEIF(NC_File.EQ.74)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'windx',NC_Var))
                            ELSEIF(NC_FILE.EQ.301)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'swan_HS',NC_Var))
                            ELSEIF(NC_FILE.EQ.302)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'swan_DIR',NC_Var))
                            ELSEIF(NC_FILE.EQ.303)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'swan_TPS',NC_Var))
                            ELSEIF(NC_FILE.EQ.307)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'swan_TMM10',NC_Var))
                            ELSEIF(NC_FILE.EQ.53)THEN
                        ...
                        IF(TRIM(ContourFileFormat2).EQ."NETCDF")THEN
                            ...
                            IF(NC_File.EQ.63)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'zeta',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.633)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'zeta_max',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.634)THEN                                                                                         
                            ...
                            ELSEIF(NC_File.EQ.64)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'u-vel',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                                CALL Check(NF90_INQ_VARID(NC_ID2,'v-vel',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,V2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.73)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'pressure',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_File.EQ.74)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'windx',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                                CALL Check(NF90_INQ_VARID(NC_ID2,'windy',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,V2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                            ELSEIF(NC_FILE.EQ.301)THEN
                                CALL Check(NF90_INQ_VARID(NC_ID2,'swan_HS',NC_Var))
                                CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,J/),count=(/NumNodesGlobal,1/)))
...
SUBROUTINE FindVectorScaleMag
        ...
        ELSEIF(TRIM(VectorFileFormat).EQ."NETCDF")THEN
            ...
            NC_Status = NF90_INQ_VARID(NC_ID,'u-vel',NC_Var)
            IF(NC_Status.EQ.NF90_NOERR)THEN
                VectorFileNumCols = 2
                NC_File = 64
            ENDIF
            NC_Status = NF90_INQ_VARID(NC_ID,'windx',NC_Var)
            IF(NC_Status.EQ.NF90_NOERR)THEN
                VectorFileNumCols = 2
                NC_File = 74
            ENDIF
        ...
        loopvectorscale: DO J=1,NumRecsLocal 
            ...
            ELSE
...
#ifdef NETCDF
                    NumNodes1 = NumNodesGlobal
                    IF(NC_File.EQ.64)THEN
                        CALL Check(NF90_INQ_VARID(NC_ID,'u-vel',NC_Var))
                    ELSEIF(NC_File.EQ.74)THEN
                        CALL Check(NF90_INQ_VARID(NC_ID,'windx',NC_Var))
                    ENDIF
                ...
                IF(TRIM(VectorFileFormat).EQ."NETCDF")THEN
                    ...
                    IF(NC_File.EQ.64)THEN
                        CALL Check(NF90_INQ_VARID(NC_ID,'u-vel',NC_Var))
                        CALL Check(NF90_GET_VAR(NC_ID,NC_Var,U1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                        CALL Check(NF90_INQ_VARID(NC_ID,'v-vel',NC_Var))
                        CALL Check(NF90_GET_VAR(NC_ID,NC_Var,V1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                    ELSEIF(NC_File.EQ.74)THEN
                        CALL Check(NF90_INQ_VARID(NC_ID,'windx',NC_Var))
                        CALL Check(NF90_GET_VAR(NC_ID,NC_Var,U1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                        CALL Check(NF90_INQ_VARID(NC_ID,'windy',NC_Var))
                        CALL Check(NF90_GET_VAR(NC_ID,NC_Var,V1,start=(/1,J/),count=(/NumNodesGlobal,1/)))
                    ENDIF
#endif
                ENDIF
...
SUBROUTINE ProcessFort14File
        ...
        INTEGER             :: IS,IE,ReadLoop,MaxNETCDFReadSize=1d6
...
#endif
        ENDIF
        ! William 7/13/2016, added for support to grid files in folders  
        FORT14File = Trim_Path(Fort14File)
        ...
        ENDIF
        !WJP 
        IF (LongW.eq.-999) THEN
           ! Get automatic bounds of mesh
           LongW = minval(Long)
           LongE = maxval(Long)
           LatS  = minval(Lat)
           LatN  = maxval(Lat)
...
!       ENDIF
        ...
        ELSEIF(TRIM(GridFileFormat).EQ."NETCDF")THEN
#ifdef NETCDF
            ! William 7/20/2016 - we are getting segmentation fault lets read in
            ! smaller quantities
            CALL Check(NF90_INQ_VARID(NC_ID,'element',NC_Var))
            ReadLoop = CEILING(REAL(NumElemsGlobal)/REAL(MaxNETCDFReadSize))
            IS = 1
            IE = 0
            DO I=1,ReadLoop
                IE = min(NumElemsGlobal,IE + MaxNETCDFReadSize)
                DO K=1,3
                   CALL Check(NF90_GET_VAR(NC_ID,NC_Var,NC(K,IS:IE),[K,IS],[1,IE-IS+1]))
                ENDDO
                IS = IS + MaxNETCDFReadSize
            ENDDO  
#endif
        ENDIF
...
SUBROUTINE ReadInputFile
        ...
        INQUIRE(FILE=TRIM(TempPath),EXIST=FileExists)
        IF(.NOT.FileExists)THEN
            IF(MyRank.EQ.0)THEN
...
SUBROUTINE ReadTimeStamp(UnitNumber, FileNameLen, FileName, TimeReal, TimeReal2, NonDefaultNodes, DefaultValue)
        ...
        USE DATA, ONLY: MyRank, NumNodesGlobal, Verbose
        ...
        REAL,INTENT(OUT)             :: TimeReal
        REAL,INTENT(OUT)             :: TimeReal2
...
SUBROUTINE RemoveIfExists(Ext,Length)
        ...
        USE DATA, ONLY: RemoveFiles,TempPath
...
SUBROUTINE WritePSImage(Record,IL1,IL2,IL3)
 ...
 1236   FORMAT(F15.8)
        ...
        HEIGHT = LatNLocal - LatSLocal
        ProjectionC = ""
        IF ( abs(HEIGHT) < 180.0) THEN
!       IF(IfGoogle.EQ.0)THEN
...
!       ENDIF
        ...
        ENDIF
        ELSE
                HEIGHT = WIDTH * HEIGHT / (LongELocal - LongWLocal) 
                ProjectionC = "-JX"//TRIM(ADJUSTL(WidthC))//"i/0"       
        ENDIF
        IF((IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
            IF (ContourFileType(14:16).eq.'amp'.or.ContourFileType(14:16).eq.'phs') THEN
                WRITE(UNIT=PlotName,FMT='(A,A,A,A)') TRIM(AlphaLabel),"_",TRIM(Const(Record)) !,ContourFileType(14:16)
            ELSE
        ...
        ELSEIF((IfAddTimeBar.EQ.1).AND.(IfPlotVectors.EQ.0))THEN
            ScaleHeight = Height - 1.0 - SideBarTriangleHeight
        ...
        IF(IfPlotBackgroundImages.EQ.1)THEN
...
11311       CONTINUE
            ...
            IF(AddBackgroundImage)THEN
                ...
                WRITE(Line,'(A)') "convert "//TRIM(TempPath)//TRIM(GeoRefFilePNG) &
                        //" "//TRIM(TempPath)//TRIM(GeoRefFileEPS)
                CALL SYSTEM(TRIM(Line))
        ...
        IF(IfPlotFilledContours.GE.1)THEN
            ...
            IF(OptimizeContours.EQ.0)THEN
                ...
                IF(.NOT.WroteBorder.AND.(IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
                    ...
                    ELSE
                           Line = TRIM(Line)//" "//"-BWS"//TRIM(ADJUSTL(BorderIncrementMajorC))//&
                                              "/"//TRIM(ADJUSTL(BorderIncrementMinorC))
                    ENDIF
                ENDIF
                Line = TRIM(Line)//" "//"-I"
                IF (ProjectionC(3:3) /= 'X') then 
                    Line = TRIM(Line)//" "//"-T"//TRIM(TempPath)//TRIM(Trim_Path(Fort14File))//".tri"
                ENDIF
...
!                ENDIF
        ...
        IF(((IfPlotFilledContours.GE.1).OR.((IfPlotContourLines.GE.1).AND.(INDEX(ColorLines,"CONTOUR").GT.0)).OR. &
           ...
           (TRIM(ContourFileType).EQ."HWM-CSV")).AND.(INDEX(ColorLines,"GRID-DECOMP").LE.0).AND.(IL1.EQ.1))THEN
            ...
            IF(INDEX(Palette,"INTERVALS").LE.0)THEN
                IF (ScaleLabelEvery.EQ.-9999d0) then
                   !logarithmic
...
!                Line = TRIM(Line)//" "//"-Sc10p"
 ...
 9724       CONTINUE
        ...
        IF(IfAddPlotLabel.EQ.1)THEN
            ...
            IF (ContourFileType(14:16).eq.'amp'.or.ContourFileType(14:16).eq.'phs') THEN
               L = LEN_TRIM(PlotLabel)
               PlotLabel = TRIM(PlotLabel)//" "//TRIM(const(Record)) !//" "//TRIM(ContourFileType(14:16)
            ENDIF
            ...
            IF (ContourFileType(14:16).eq.'amp'.or.ContourFileType(14:16).eq.'phs') THEN
                ! Reset
                PlotLabel = PlotLabel(1:L)
            ENDIF

        ENDIF

        IF((IfAddTimeBar.EQ.1).AND.(IfGoogle.EQ.0).AND.(IfGIS.EQ.0))THEN
            ...
            !William 720/2016 Change time units depending on range
            write(*,*) 'New time info',timeL
            IF (timeL >= 259200.0) THEN
                TimeVal = 86400.0
                TimeUnit = " days"
            ELSEIF (timeL < 18000.0) THEN
                TimeVal = 60.0
                TimeUnit = " mins"
            ELSE
                TimeVal = 3600.0
            ...
            WRITE(UNIT=27,FMT='(A,F5.2,A)') "0 0 14 0 0 LT ",CurrentTime/TimeVal,TimeUnit
...
!                                Energy flux as a scalar
                ...
                ELSEIF((TRIM(ContourFileType).EQ."13-MANNING").OR.           &
                       ...
                       (TRIM(ContourFileType).EQ."13-REFRAC").OR.            &
                       (TRIM(ContourFileType).EQ."13-ITFRIC").OR.            &
                       (TRIM(ContourFileType).EQ."13-FRIC").OR.              &
                       (TRIM(ContourFileType).EQ."13-ADVS")) THEN
                    ...
                    DO I=1,NumAttributes1
                        ...
                        ELSEIF((INDEX(AttributeLabel,"quadratic_friction_coefficient_at_sea_floor").GT.0).AND.(TRIM(ContourFileType).EQ."13-FRIC"))THEN
     ...
     &                         .AND.(TRIM(ContourFileType).EQ."13-REFRAC"))THEN
                        ...
                        ELSEIF((INDEX(AttributeLabel,"internal_tide_friction").GT.0).AND.(TRIM(ContourFileType).EQ."13-ITFRIC"))THEN
                                                                  
                            READ(UNIT=19,FMT='(A)') JunkC
                            READ(UNIT=19,FMT=*) Column_No
                            READ(UNIT=19,FMT=*) AttributeDefault 

                        ELSEIF((INDEX(AttributeLabel,"advection_state").GT.0).AND.(TRIM(ContourFileType).EQ."13-ADVS"))THEN
...
!                                Energy flux as a scalar
                    ...
                    IF(TRIM(ContourFileFormat2).EQ."NETCDF")THEN
                        ...
                        IF(NC_File.EQ.63)THEN
                            CALL Check(NF90_INQ_VARID(NC_ID2,'zeta',NC_Var))
                            CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,Record/),count=(/NumNodesGlobal,1/)))
                        ELSEIF(NC_File.EQ.633)THEN
                            CALL Check(NF90_INQ_VARID(NC_ID2,'zeta_max',NC_Var))
                            CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,Record/),count=(/NumNodesGlobal,1/)))
                        ELSEIF(NC_File.EQ.634)THEN
                        ...
                        ELSEIF(NC_File.EQ.64)THEN
                            ...
                            CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,Record/),count=(/NumNodesGlobal,1/)))
                            CALL Check(NF90_INQ_VARID(NC_ID2,'v-vel',NC_Var))
                            CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,V2,start=(/1,Record/),count=(/NumNodesGlobal,1/)))
                        ELSEIF(NC_File.EQ.73)THEN
                            CALL Check(NF90_INQ_VARID(NC_ID2,'pressure',NC_Var))
                            CALL Check(NF90_GET_VAR(NC_ID2,NC_Var,U2,start=(/1,Record/),count=(/NumNodesGlobal,1/)))
                        ELSEIF(NC_File.EQ.74)THEN
     ...
     &                         .AND.(TRIM(ContourFileType).EQ."13-REFRAC"))THEN
        ...
        IF(IfPlotParticles.GT.0)THEN
            ...
            ELSEIF(TRIM(ParticleFileFormat).EQ."NETCDF")THEN
                ...
                CALL Check(NF90_INQUIRE_VARIABLE(NC_ID1,NC_Var,dimids=NC_DimIDs))
                CALL Check(NF90_INQUIRE_DIMENSION(NC_ID1,NC_DimIDs(1),len=NumParticles))
        ...
        IF(IfPlotVectors.EQ.1)THEN
            ...
            VectorUFile = Trim_Path(VectorUFile)
            VectorVFile = Trim_Path(VectorVFile)    
...
FUNCTION Trim_Path(Pathname)
        implicit none
        character*40 :: Trim_Path
        character*40,intent(in) :: Pathname
        integer :: i, P_Length
        P_Length  = 40 !LEN(TRIM(Pathname))
        Trim_Path = Pathname
        do i = P_Length,1,-1
                if (Pathname(i:i) == '/') then
                        Trim_Path = Pathname(i+1:P_Length)
                        exit
                endif
        enddo        
END FUNCTION
